...
export class GsmControllerService {
  ...
  openGSM(gsm : GSMController, userID : number) : Observable<String>{
    return this.http.put<String>(this.url+"/open/"+userID+"/open",gsm);
  }
  closeGSM(gsm : GSMController, userID : number) : Observable<String>{
    return this.http.put<String>(this.url+"/close/"+userID+"/close",gsm);
  }
